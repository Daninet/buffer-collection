...
class BufferCollection {
  ...
  indexOf (value, startOffset = 0) {
    // console.log('indexOf', value, startOffset, this);
    ...
    if (startOffset + bufferValue._length >= this._length) {
      return -1;
    }
  ...
  lastIndexOf (value, startOffset) {
    // console.log('lastIndexOf', value, startOffset, this);
    let bufferValue = value;
    if (!(value instanceof BufferCollection)) {
      bufferValue = BufferCollection.from(value);
    }
  ...
  toString (encoding, start, end) {
    return this._bufs.map((buf) => buf.toString(encoding, start, end)).join('');
  }
